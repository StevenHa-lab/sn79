...
class MinerAgent1(FinanceSimulationAgent):
    ...
    def regime_detection(self, validator_hotkey, book_id):
        ...
        if len(prices) < 5:
            return "neutral"
        if status[book_id] == 0:
            if bids[-2] == max(bids) and bids[-1] < bids[-2]:
                self.trime_price(validator_hotkey, book_id)
                status[book_id] = 2
                return "sell"
            elif asks[-2] == min(asks) and asks[-1] > asks[-2]:
                self.trime_price(validator_hotkey, book_id)
                status[book_id] = 1
                return "buy"
            else:
                return "neutral"
        elif status[book_id] == 1:
            if (bids[-2] == max(bids) and bids[-1] > asks[0] + 0.3) or bids[-1] < asks[0] - 1:
                self.trime_price(validator_hotkey, book_id)
                status[book_id] = 0
                return "sell"
        elif status[book_id] ==2:
            if (asks[-2] == min(asks) and asks[-1] < bids[0] - 0.3) or asks[-1] > bids[0] + 1:
                self.trime_price(validator_hotkey, book_id)
                status[book_id] = 0
                return "buy"
        else:    
    ...
    def respond(self, state):
        ...
        for book_id, book in state.books.items():
            ...
            if(validator_hotkey == "5EWwdZB7qCCMaAso5Mzcks4UUcPxKYvpAj32t5Mg1v6HSxoF"):
                print(f"book_id: {book_id}, qty: {qty}, trend: {trend}")
